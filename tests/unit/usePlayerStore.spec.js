...
describe('usePlayerStore', () => {
	...
	it('should toggle allow_select status of a player', async () => {
		...
		await playerStore.togglePlayerField('player1', 'allow_select');
	...
	it('should add a new player successfully', async () => {
		...
		//expect(collection).toHaveBeenCalledWith(db, 'players')
	...
	it('should handle error when adding a new player-getdocs empty is false', async () => {
		const playerStore = usePlayerStore();
		...
		// 调用 addPlayer 方法
		//await playerStore.addPlayer(newPlayer);
		await expect(playerStore.addPlayer(newPlayer)).rejects.toThrow('玩家已存在，不能重复添加。');
		// 检查是否调用了 alert 和 console.error
	...
	it('should handle error when adding a new player-addDoc have error', async () => {
		const playerStore = usePlayerStore();
		...
		// 调用 addPlayer 方法
		//await playerStore.addPlayer(newPlayer);
		await expect(playerStore.addPlayer(newPlayer)).rejects.toThrow('Firestore error');
		// 检查是否调用了 alert 和 console.error
		expect(console.error).toHaveBeenCalledWith('Error adding player:', expect.any(Error));
