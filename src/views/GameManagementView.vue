<template>
  <div class="game-management">
    ...
    <div ref="gameInfo" class="print-container" v-else-if="gameStore.currentGame && !isLoading">
      <!-- 显示当前局信息 -->
      ...
      <v-row>
        <v-col cols="12" md="8">
          <GameInfoDisplay>
            <template v-slot:btn>
              <v-btn color="cyan-darken-1" @click="reportList()" size="small" icon variant="flat"
      ...
      />
      <v-divider :thickness="4" class="border-opacity-75 mt-10"></v-divider>
      ...
      <LogsTable v-else />
      <!-- 打印按钮 -->
      <v-btn v-if="gameStore.currentGame" color="primary" @click="printGameInfo"
        >打印游戏信息</v-btn
      >
    </div>
...
const reportList = async () => {
  await voiceReportList(gameStore.players)
}
</script>
