  ...
  <div class="summary-table">
    ...
    >
      <template #item.is_zero="{ item }">
        <v-chip
          :color="item.is_zero ? 'green-darken-3' : 'red'"
          dark
          variant="flat"
          v-if="isStarted"
          >{{ item.is_zero ? '是' : '否' }}</v-chip
        >
      </template>
      ...
      <template #item.save_summary="{ item }">
        <v-btn
          v-if="item.total_players > 0 && item.is_game_completed"
          color="primary"
          @click="saveSummary"
          >保存总结</v-btn
        >
    ...
    <v-alert v-if="gameStore.showAlert" type="success" class="mt-2"> 总结保存成功！ </v-alert>
  </div>
</template>
<script>
// @ts-ignore
